...
22. Do not add AI attribution, AI co-author trailers, or tool/vendor co-author
    lines in commits, manifests, docs, generated metadata, or branch names. The
    branch-name limb applies prospectively to newly created branches; the
    pre-existing owner-approved `codex/reaction-otel` branch is a recorded
    exception and must not be renamed.
